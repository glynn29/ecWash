...
const AddToCartCard = ({item, onAddItem, close}) => {
    ...
    const actions = (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%'}}>
            <Button style={{width: '45%'}} size={"small"} onClick={() => onAddItem(item, intAmount)} variant={"contained"} color={"secondary"}>Add</Button>
            <input style={{width: '45%'}} type={"number"} step={1} min={1} max={100} onChange={(e) => handleInputChange(e)} value={amount} />
        </div>
    );
    ...
    return (
        <Container component="main" maxWidth="md" style={{textAlign: 'center', padding: 2, width: '90%'}}>
            ...
            <Grid container spacing={2}>
                ...
                <Grid container item xs={12} sm={6} spacing={2} style={{margin: '0 auto'}}>
                    <Grid item xs={12}>
                        <Typography variant={"body2"}>SKU: {item.code}</Typography>
                    </Grid>
                    <Grid item xs={12}>
                        <Typography variant={"body2"}>Description: {item.details}</Typography>
                    </Grid>
                    <Grid item xs={12}>
                        <FormControl style={{width: '100%'}}>
                            <TextField type={"number"}
                                       onChange={(e) => handleInputChange(e)}
                                       value={amount} variant={"outlined"}
                                       label={"Quantity"}
                                       InputProps={{inputProps:{min: 1, max: 50, step: 1}}} />
                        </FormControl>
                    </Grid>
                    <Grid item sm={6} xs={12}>
                        <Button onClick={() => close()} variant={"outlined"} style={{width: '100%', color: '#d8222b'}}>Close</Button>
                    </Grid>
                    <Grid item sm={6} xs={12}>
                        <Button type={"submit"} variant={"contained"} color={"primary"} style={{width: '100%'}}>Add to Cart</Button>
                    </Grid>
                </Grid>
            </Grid>
            </form>
